...
def load_first_2d_fits_image(path):
    ...
    raise ValueError("No 2D image HDU was found in %s" % str(path))
...
def plot_simple_2d_image_panel(ax, record):
    science_frame_path = Path(str(record.get("science_frame", ""))).expanduser()
    if science_frame_path.is_file():
        image_data = load_first_2d_fits_image(science_frame_path)
    else:
        image_data = np.asarray(record["frame_data"], dtype=float)

    finite_image = image_data[np.isfinite(image_data)]
    if finite_image.size == 0:
        norm = None
        display_vmin, display_vmax = 0.0, 1.0
    else:
        positive = finite_image[finite_image > 0]
        if positive.size > 0:
            display_vmin, _ = ZScaleInterval(contrast=SUMMARY_ZSCALE_CONTRAST).get_limits(positive)
            if not np.isfinite(display_vmin) or display_vmin <= 0:
                display_vmin = float(np.nanpercentile(positive, 1.0))
            display_vmax = float(np.nanmax(positive))
            if not np.isfinite(display_vmax) or display_vmax <= display_vmin:
                display_vmax = max(display_vmin * 10.0, display_vmin + 1.0)
            norm = LogNorm(vmin=display_vmin, vmax=display_vmax)
        else:
            display_vmin = float(np.nanmin(finite_image))
            display_vmax = float(np.nanmax(finite_image))
            if not np.isfinite(display_vmax) or display_vmax <= display_vmin:
                display_vmax = display_vmin + 1.0
            norm = None

    y_pixels = np.arange(image_data.shape[0], dtype=float)
    x_pixels = np.arange(image_data.shape[1], dtype=float)
    y_edges = centers_to_edges(y_pixels)
    x_edges = centers_to_edges(x_pixels)
    ...
    image = ax.imshow(
        ...
        cmap=SUMMARY_IMAGE_CMAP,
        norm=norm,
        vmin=None if norm is not None else display_vmin,
        vmax=None if norm is not None else display_vmax,
        extent=[
            float(np.nanmin(x_edges)),
            float(np.nanmax(x_edges)),
            float(np.nanmin(y_edges)),
            float(np.nanmax(y_edges)),
        ],
        rasterized=True,
    )
    ax.set_xlabel("X pixel")
...
def create_single_panel_figure(kind, bundle):
    ...
    elif kind == "simple_2d_image":
        plt.close(fig)
        fig, ax = plt.subplots(figsize=(7.2, 7.2))
        image = plot_simple_2d_image_panel(ax, bundle["record"])
        ax.set_box_aspect(1)
...
def save_summary_figures(bundle, output_dir=None, dpi=300, include_simple_2d_image=False):
    ...
    panel_paths = {}
    panel_kinds = ["raw_frame", "row_profile", "spectrum"]
    if include_simple_2d_image:
        panel_kinds.append("simple_2d_image")
...
def main():
    ...
    parser.add_argument("--simple-2d-image", action="store_true", help="Also save/show a plain 2D spectral image with detector x/y axes and a counts color bar.")
    ...
    saved = save_summary_figures(
        bundle,
        output_dir=args.output_dir,
        dpi=max(int(args.dpi), 72),
        include_simple_2d_image=args.simple_2d_image,
    )
    ...
    if args.show:
        ...
        plt.show()
        plt.close(fig)
        if args.simple_2d_image:
            fig2 = create_single_panel_figure("simple_2d_image", bundle)
            plt.show()
            plt.close(fig2)
