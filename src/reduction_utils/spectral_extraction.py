    ...
    for i, row in enumerate(frame):
        ...
        if sum(row[aperture_left_hand_edge:aperture_right_hand_edge]) <= 0:
            ...
            if instrument == "Keck/NIRSPEC": # we include dark current but not scintillation
                ...
                # Interpolated subpixels are correlated, so divide the summed variance by the oversampling factor.
                error.append(np.sqrt(np.sum(error_frame[i][aperture_left_hand_edge:aperture_right_hand_edge]**2)/oversampling_factor))
