...
def extract_trace_flux(frame,trace,aperture_width,background_offset,background_width,pre_flat_frame,poly_bg_order,am,exposure_time,verbose,star,mask,instrument,row_min,gauss_std,readout_speed,co_add_rows,rectify_frame,oversampling_factor,gain_file,readnoise_file,frame_no=0):
    ...
    # Diagnostic storage: full background-subtracted frame for this star+frame
    diag_enabled = 'SAVE_DIAG' in globals() and SAVE_DIAG
    if diag_enabled:
        # Full 2D background-subtracted image in detector coordinates
        bkg_sub_frame = np.full((nrows, ncols), np.nan, dtype=float)
        # Per-row aperture edges in detector x-coordinates
        ap_left_arr = np.full(nrows, np.nan, dtype=float)
        ap_right_arr = np.full(nrows, np.nan, dtype=float)
